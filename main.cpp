...
struct job_spec_t {
   std::atomic<job_status_t>status;
...
DWORD job_queue_worker(void * param) {
   while (job_queue.running)
   {
      ...
      {
         job_status_t expected = JOB_AVAILABLE;
         if (job_queue.specs[i].status.compare_exchange_strong(expected, JOB_PENDING)) {
            job_queue.specs[i].handler(job_queue.specs[i].params);
            job_queue.specs[i].status.store(JOB_FREE, std::memory_order_release);
         }
      }
   }
...
void job_queue_push(job_handler_t handler, void * data, uint32_t data_len)
{
   ...
   {
      job_status_t expected = JOB_FREE;
      if (job_queue.specs[i].status.compare_exchange_strong(expected, JOB_IN_PREP)) {
         job_queue.specs[i].handler = handler;
         memcpy_s(job_queue.specs[i].params, JOB_QUEUE_PARAMS_SIZE, data, data_len);
         job_queue.specs[i].status.store(JOB_AVAILABLE, std::memory_order_release);
         queued = true;
...
void job_queue_wait_until_done()
{
   ...
   while (!all_done) {
      ...
      {
         if (job_queue.specs[i].status.load(std::memory_order_acquire) != JOB_FREE) {
            all_done = false;
         } 
      }
   }
